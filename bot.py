...
and replies with exactly one JSON object shaped as the question asks:

    {"answer": <inner answer>, "log_url": "<public URL to the run's JSONL log>"}

The exam (question.md) explicitly requires this wrapper — e.g.
`{"answer": {"state": "Assam"}, "log_url": "https://..."}` — with log_url a
public, wget-able URL to the agent's JSONL log. The grader extracts the
`answer` field and checks `log_url` is reachable; it does NOT exact-match the
whole object (log_url varies per run).
...
def _final_reply(answer_value: Any, log_url: str) -> str:
    """The Telegram reply for a JSON-answer question, shaped as question.md
    requires: {"answer": <inner answer>, "log_url": <public JSONL log URL>}.

    The grader extracts the `answer` field for exact-match and checks
    `log_url` is a reachable URL; it does not exact-match the whole object.
    """
